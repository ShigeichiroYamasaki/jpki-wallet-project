...
更新日：2026年9月7日。[仕様書総目次](/specification) / [プロトタイプ系](/prototype-specification)
...
継続認証には新規challengeを発行し、登録nonceを再利用しない。重要操作は[共通仕様16節](/specification#evidence-design)のOperationIntent、操作別WebAuthn assertion、EIP-712署名を使用する方針とする。SIWEログインのみで著作権・原盤権等の移転・許諾を承認しない。
...
模倣シナリオの成功だけでは上記を満たさない。[旧仕様](/specification-legacy)は検討履歴として参照し、本番提供の証拠には使用しない。
