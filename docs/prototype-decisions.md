...
仕様v0.4を本番系とプロトタイプ系に分離する。プロトタイプ端末はユーザPC（macOS）のブラウザと接続したカードリーダーとし、ADR-009のモバイル中心案を今回の必須構成から外す。本番端末は検証後に決定する。
...
既存GCEの停止・資源制約を維持し、ユーザPC（macOS）内だけで動くNode＋SQLiteプロファイルを追加する。Originはlocalhost:18080に固定し、Host・Origin・CSRF・HttpOnly/SameSite・一回限りchallengeで保護する。カード処理はローカル子プロセスで行い、PIN・証明書・署名原本をHTTPへ返さない。ADR-022のNative Messaging候補に代わる、今回の限定実装である。
...
## ADR-024：参加者のカード体験を仮想化する

- 状態：採用（2026-09-08）
- 決定：参加者向けプロトタイプは実マイナンバーカードを利用しない。架空カードをセットし、読み取りを模倣する画面を提供する。
- 実装：カード操作はブラウザ内、PF応答は既存mock API。パスキー・ウォレット署名の暗号検証は継続する。
- 境界：実カード、PIN、個人番号、NFC、デジタル庁APIは使わない。ブラウザ内の体験完了を本人確認成功と表示しない。
- 従来の実カード試験は履歴として保存し、通常起動のカードAPIは無効化する。
