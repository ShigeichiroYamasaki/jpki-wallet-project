...
::: warning 履歴資料・現行仕様ではありません
本番・プロトタイプ分離前の記録です。現行要件は[仕様書 v0.4](/specification)を参照してください。原設計に2026年9月7日の証拠基盤補足（ADR-020・021／仕様16節）を追加しています。追加部分は未実装です。記載された製品仕様・技術的保証・法的説明は検証済みの事実を示すものではありません。
:::
...
[← トップページへ](/) · <a :href="withBase('/documents/specification-v0.3.md')" download>Markdown全文（補足含む）</a>
