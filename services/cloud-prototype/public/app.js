...
function controls(){$('card-place').disabled=busy||identity;$('card-read').disabled=busy||cardState!=='placed';for(const [id,ready] of Object.entries({register:supported&&!!session&&!session.authenticated,login:supported&&!!session,identity:session?.authenticated&&cardState==='read',wallet:identity,'wallet-sign':identity&&!!preparedSignature,operation:wallet,export:evidence}))$(id).disabled=busy||!ready}
...
$('login').onclick=()=>run(async()=>{identity=wallet=evidence=false;preparedSignature=undefined;const options=await api('passkeys/auth/options');await api('passkeys/auth/verify',await SimpleWebAuthnBrowser.startAuthentication({optionsJSON:options}));await bootstrap();say('ログインできました。本人確認の模倣へ進んでください。')})
...
$('card-read').onclick=()=>{if(busy||cardState!=='placed')return;cardState='read';$('card-reader').dataset.state='read';$('card-progress').textContent='仮想読み取り完了：サンプル利用者 A。実カード・証明書の読み取りや検証は行っていません。';controls()}
$('identity').onclick=()=>run(async()=>{if(cardState!=='read')throw new Error('先に仮想カードをセットして読み取ってください。');identity=wallet=evidence=false;preparedSignature=undefined;const result=await api('mock/verify',{person:'person-a',scenario:'valid'});identity=result.outcome==='verified';say('仮想カードの模倣結果をAPIに登録しました。ウォレットを接続してください。')})
